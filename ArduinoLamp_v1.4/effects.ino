  ...
  for (uint8_t x = 0U; x < WIDTH; x++)
  {
    ...
    if ((uint32_t)thisColor == 0U)
    {
     if (random8(0, 50) == 0U)
      ...
      if (Scale>=255) drawPixelXY(x, HEIGHT - 1U, 0xE0FFFF - 0x101010 * random(0, 4)); // Снег
      else
...
void ballRoutine()
{
  if (loadingFlag)
  {
    ...
    deltaValue = map(Scale, 0U, 255U, 2U, max((uint8_t)min(WIDTH, HEIGHT) / 3, 2));
...
void LavaLampRoutine() {
  ...
  { for (byte i = 0; i < (WIDTH / 2) -  ((WIDTH - 1) & 0x01); i++) {
      ...
      ball[i][1] = (float)random8(5, 11) / (Speed) / 4.0;
      ball[i][0] = 0;
  ...
  for (byte i = 0; i < (WIDTH / 2) -  ((WIDTH - 1) & 0x01); i++) {
    ...
    if (ball[i][0] < 0.01) {                  // почему-то при нуле появляется мерцание (один кадр, еле заметно)
      ball[i][1] = (float)random8(5, 11) / (257U - Speed) / 4.0;
      ball[i][0] = 0.01;
    }
