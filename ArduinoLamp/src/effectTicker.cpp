...
void drawLevel() {
  ...
  switch (numHold) {
    case 1:
      ind = map(Brightness, 0, 255, 0, WIDTH - 1);
  ...
  switch (numHold) {
    case 1:
      ind = map(Brightness, 0, 255, 0, HEIGHT - 1);
      ...
      break;
    case 2:
      ind = map(Speed, 0, 255, 0, HEIGHT - 1);
      for (byte y = 0; y <= HEIGHT ; y++) {
      ...
      break;
    case 3:
      ind = map(Scale, 0, 255, 0, HEIGHT - 1);
      for (byte y = 0; y < HEIGHT ; y++) {
